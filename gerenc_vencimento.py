...
def criar_botao_remover(index):
    return tk.Button(root, text="Remover", command=lambda i=index: remover_produto(i))
    ...
    try:
        with open('dados_produtos.json', 'r') as file:
            ...
            for produto in dados:
                produto['data_vencimento'] = datetime.strptime(produto['data_vencimento'], '%d/%m/%Y').strftime('%d/%m/%Y')
            return dados
